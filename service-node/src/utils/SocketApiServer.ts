...
export interface ServerConnectionEventsType {
  ...
  clientConnected(socketUid: string, clientIp: string, clientComm: ClientCommProxyType): void;
...
export class SocketApiServer {
  ...
  constructor(sio_path: string, corsConfig: object, httpServer: HttpServer, apiConnectionsListener: ServerConnectionEventsType) {
    ...
    this.socketIoServer.on('connection', (clientSocket) => {
      ...
      // log
      const clientIp = (clientSocket.handshake.headers["x-forwarded-for"] as string) || clientSocket.handshake.address;
      const dateString = new Date().toISOString().substring(5,16).replace('T',' ');
      if (LOG_EVENTS) log(` > ${socketUid} [${Object.keys(this.currentClients).length}] ${clientIp}  ${dateString}`);
      ...
      apiConnectionsListener.clientConnected(socketUid, clientIp, clientCommProxy);
    });
  }
